  ...
  if (!analysis) {
    return (
      <section className="surface-panel p-6">
        ...
        <div className="mt-4 rounded-xl border border-dashed border-line/50 bg-surface/30 p-10 text-center">
  ...
  return (
    <div className="grid min-w-0 gap-5">
      <section className="surface-panel p-6">
        ...
        {parserNotes.length ? (
          <div className="mt-4 rounded-xl border border-line/60 bg-surface/50 px-4 py-3 text-sm leading-6 text-muted">
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1.1fr),minmax(0,0.9fr)]">
        <section className="surface-panel p-6">
          ...
          <div className="mt-5 space-y-3">
            {expenseRows.map((row) => (
              <div key={row.label} className="flex items-center justify-between rounded-xl border border-line/60 bg-surface/50 px-4 py-3">
                <span className="text-sm text-ink">{row.label}</span>
                <span className="font-mono text-sm tabular-nums text-muted">{currency(row.value)}</span>
              </div>
            ))}
          </div>
        </section>
            ...
            <div className="mt-5 grid gap-3">
              {analysis.strengths.length ? (
                analysis.strengths.map((strength) => (
                  <div key={strength} className="rounded-xl border border-green/20 bg-green-soft p-4 text-sm leading-6 text-green">
                    {strength}
                  </div>
                ))
              ) : (
                <div className="rounded-xl border border-line/60 bg-surface/50 p-4 text-sm leading-6 text-muted">
            ...
            <div className="mt-5 grid gap-3">
              {analysis.risks.length ? (
                analysis.risks.map((risk) => (
                  <div key={risk} className="rounded-xl border border-red/20 bg-red-soft p-4 text-sm leading-6 text-red">
                    {risk}
                  </div>
                ))
              ) : (
        ...
        <section className="surface-panel p-6">
          ...
          <div className="mt-5 grid gap-3">
            {estimatedNotes.map((note) => (
              <div key={note} className="rounded-xl border border-gold/20 bg-gold-soft p-4 text-sm leading-6 text-gold">
                {note}
              </div>
            ))}
          </div>
